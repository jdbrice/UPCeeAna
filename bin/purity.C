...
void purity( float mchi = 10, float max_x = 30, bool exp2=false, float c1=30, float c2=120 ){
	...
	hchiee->Draw();
	hchiee->GetXaxis()->SetRangeUser(0, max_x);
	hchiee->Draw("same pe");
	...
	tl->SetTextFont(42);
	tl->DrawLatexNDC( 0.5, 0.7, TString::Format( "purity ( #chi^{2}_{ee} < %d ) = %0.2f%%", (int)mchi, p*100 ) );
