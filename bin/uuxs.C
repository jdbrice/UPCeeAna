...
void uuxs(){
	...
	hMass->Scale( 1.08 / ( 0.6 ) );
	// hMass->Scale( 0.217568 );
	...
	hMass->Scale( 1.0, "width" ); // VtxLoss and dm
	hMass->Scale( 1.0 / 0.82 ); // tof PID Eff
	hMass->Scale( 1.0 / 0.97 ); // chi^2_ee Eff
	// hMass->Scale( 1 / 1.12 );

	if ( "Run10" == Run ){
		hMass->Scale( 1.0 / 679000.0 ); // Lumi
	}
	...
	hSLrap->GetXaxis()->SetTitle( "M_{ee} (GeV/c^{2})" );
	hSLrap->GetYaxis()->SetTitle( "#frac{d#sigma(XnXn)}{dM} (mb)" );
	...
	hrapid2->Scale( 1.0 / 0.82 ); // tof PID Eff
	hrapid2->Scale( 1.0 / 0.97 ); // chi^2_ee Eff
	// hrapid2->Scale( 1 / 1.12 );

	if ( "Run10" == Run ){
		hrapid2->Scale( 1.0 / 679000.0 ); // Lumi
	}
	...
	hPt->Scale( 1.0 / 0.82 ); // tof PID Eff
	hPt->Scale( 1.0 / 0.97 ); // chi^2_ee Eff
	// hPt->Scale( 1 / 1.12 );

	if ( "Run10" == Run ){
		hPt->Scale( 1.0 / 679000.0 ); // Lumi
	}
	if ( "Run12" == Run ){
	...
	hpt2id2->Scale( 1.0 / 0.82 ); // tof PID Eff
	hpt2id2->Scale( 1.0 / 0.97 ); // chi^2_ee Eff

	if ( "Run10" == Run ){
		hpt2id2->Scale( 1.0 / 679000.0 ); // Lumi
	}
